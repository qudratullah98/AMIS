...
export default function OutCarsMultiLevelBarChart({ data }) {
    ...
    const [allLevelData, setAllLevelData] = useState({
        things: [
            ...
            [
                t("largeVehicle"),
                data.OutgoingVehicleIncomsForToDay[0].total_income,
                "things",
                "getProvencesIncome",
                data.OutgoingVehicleIncomsForToDay[0].total_income,
                "async",
        ...
        try {
            ...
            if (groupId === "getProvencesIncome") {
                const response = await axios.get(
                    route("dashboard.allLargeVehiclesRevenueGraphForToDay")
                );
            ...
            if (groupId === "getIncomByTerminal") {
                ...
                const response = await axios.get(
                    route("dashboard.getToDayIncomByTerminal", {
                        province: provinceId,
                    })
                );
                fetchedData = response.data.map((item) => [
