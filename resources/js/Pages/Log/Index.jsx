...
const FilterForm = ({ description, setDescription, causer, setCauser, onFilter, onReset }) => (
  <form onSubmit={onFilter} className="flex flex-col md:flex-row items-center gap-1">
    <input
      type="text"
      value={description}
...
const LogIndex = () => {
  ...
  const handleFilter = (e) => {
    e.preventDefault();
    router.get(route('log.index'), { description, causer }, { preserveState: true, replace: true });
  };

  const resetFilters = () => {
    setDescription('');
    setCauser('');
    router.get(route('log.index'), {}, { preserveState: true, replace: true });
  };
  ...
  return (
    <AuthenticatedLayout>
      ...
      <DataTable columns={columns} links={logs.links} header="لیست لاگ‌ها" addButton={false} enableSearch={false}>
        {logs.data.map((item, i) => (
          <tr key={item.id || i} className="hover:bg-slate-100 transition-all duration-300 ease-in-out">
            <td className="p-3 m-3 text-center text-sm text-gray-600">{item.id}</td>
            <td className="p-3 m-3 text-center text-sm text-gray-600">{item.description}</td>
            <td className="p-3 m-3 text-center text-sm text-gray-600">{item.causer?.name || 'System'}</td>
              ...
              <div className=" text-right mx-auto">
                ...
                {item.properties?.old && (
                  <div className="bg-red-50 border-l-4 border-red-600 p-1 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out">
                    <div className="flex items-center">
                      ...
                      <span className="text-red-700 font-semibold text-md">قدیم</span>
                    </div>
                ...
                {item.properties?.attributes && (
                  <div className="bg-green-50 border-l-4 border-green-600 p-1 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out">
                    <div className="flex items-center">
                      <span className="text-green-700 font-semibold text-md">جدید</span>
                    </div>
