...
function EditCourse({ course, onSubmitSuccess, onCancel }) {
    ...
    const fetchCourseTypes = async () => {
        try {
            const response = await axios.get(route('course-types.list'));
    ...
    const validationSchema = Yup.object({
        ...
        code: Yup.string()
            ...
            .matches(/^[A-Z0-9-]+$/, t("validation.codeFormat")),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting, setErrors }) => {
            try {
                const response = await axios.put(route('courses.update', course.id), values);
                if (onSubmitSuccess) {
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <CustomSelect
                        ...
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 outline-none bg-white ${
                            formik.touched.course_type_id && formik.errors.course_type_id 
                                ? 'border-red-500' 
                                : 'border-gray-300'
                        }`}
                        disabled={loading}
                ...
                <div>
                    ...
                    />
                    <InputError
                        message={formik.touched.duration_hours && formik.errors.duration_hours}
                        className="mt-1.5 text-sm text-red-500"
                    />
                </div>
                ...
                <div>
                    <InputLabel
                        value={t("courses.validityMonths")}
                        required={true}
                        className="text-sm font-medium text-gray-700 mb-1.5 block"
                    />
                    <TextInput
                        ...
                        min="1"
                        className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 outline-none ${
                            formik.touched.validity_months && formik.errors.validity_months 
                                ? 'border-red-500' 
                                : 'border-gray-300'
                        }`}
                    />
